...
iOS and Android. The benchmark measures run throughput, pipeline latency, and
landmark accuracy for multiple backend and quantization configurations.
\end{chapterabstract}
...
\begin{figure}[htbp]
    \centering
    \caption{Face-processing pipeline on a real AFW sample image: the first panel shows the input crop, the second panel draws the face-detection bounding box from a benchmark result, and the third panel overlays the predicted landmarks from the same result.}
...
the image as it enters the application. The second panel shows face detection:
the system answers the question ``where is the face?'' by drawing a rectangle
around the face region. In this figure, the rectangle is taken from an exported
mobile benchmark image record for the AFW sample. This rectangle is then used to
crop and normalize the face before landmark estimation. The third panel shows
the landmarks predicted by the same benchmark record: instead of only locating
the face, the model predicts semantically meaningful points such as eyes, nose,
mouth, and face contour positions. These points are what make downstream tasks
such as face alignment, expression analysis, augmented-reality overlays, and pose
estimation possible.
...
For each run, this metric is extracted from per-iteration landmark-inference
samples and summarized in milliseconds by average, $p50$, and $p95$. For
charting, values are aggregated per configuration group using the median across
repeated runs:
\begin{equation}
...
Batch size one (BS=1) charts compare median run FPS by landmark configuration.
Batch-scaling charts plot median run FPS as a function of resolved landmark batch size
...
The focus phone is a mid-range Android device with an Arm Cortex-A76-based CPU and Mali-G76 GPU. 
The PC reference is included to show the gap between desktop and mobile execution, while the focus phone is included to show the practical performance of the models on a real mobile device.
Additional mobile-focused figures and summary tables include iPhone 14 results so that the
reader can compare all three execution environments. Direct PC+iPhone+Redmi
comparisons are shown as tables because the PC reference often dominates a
shared chart scale. The BS=1 cross-device tables include every available
backend/variant row from the summarized benchmark data, while the later
batch-scaling tables keep the comparison to XNNPACK INT8 because batching is
only meaningful between matching dynamic-batch artifacts.

\section{Batch size 1 speed and latency overview}

\begin{figure}[htbp]
    \centering
    \caption{BS=1 median run FPS: PC vs the focus Android device. Here FPS is measured benchmark-run throughput, not landmark-forward-pass-only FPS; higher bars are better.}
...
\begin{figure}[htbp]
    \centering
    \caption{BS=1 median landmark-inference latency: PC vs the focus Android device. This measures only the landmark model forward pass in milliseconds, so lower bars are better.}
...
gap between desktop and mobile execution at BS=1. Batch size one means that the
...
one. The FPS chart should be read as a measured run-throughput indicator: if FPS is
low, the application cannot update landmark predictions smoothly. The latency
chart should be read as a model-level diagnostic: it shows whether the landmark
network itself is expensive, even if the full pipeline also contains detection,
cropping, and post-processing.

The PC reaches roughly 16--25 run FPS across the main landmark models, while the
focus phone falls into a narrower 3--8 run-FPS band. The gap is not uniform. It is
smallest for the lightweight PFLD-MNv2-0.25 (PC/phone $\approx$~2.0$\times$ in
run FPS) and largest for PipNet-R18 in FP32, where the PC is roughly an order of
magnitude faster ($24.86$ vs $2.28$ run FPS, i.e.\ $\sim$10.9$\times$). The
practical implication is that a model that looks comfortable on a desktop can
still be too slow on a phone unless the mobile backend, quantization, and
end-to-end pipeline are measured directly.
...
\begin{table}[htbp]
\centering
\caption{BS=1 measured run throughput across all available device/backend variants; higher run FPS is better. Bold marks the best value in each row.}
\label{tab:cross-device-bs1-fps}
\scriptsize
...
\toprule
Model & \makecell{PC\\{}PyTorch} & \makecell{PC\\{}XNN FP32} & \makecell{PC\\{}XNN INT8} & \makecell{iPhone\\{}Core ML} & \makecell{iPhone\\{}XNN FP32} & \makecell{iPhone\\{}XNN INT8} & \makecell{iPhone\\{}Portable} & \makecell{Redmi\\{}XNN FP32} & \makecell{Redmi\\{}XNN INT8} \\
\midrule
MobileFaceNet      & 5.49 & 16.54 & \best{16.75} & 8.45 & 5.14 & 5.38 & 0.30 & 3.39 & 3.46 \\
PFLD-MNv2-0.25     & -- & \best{16.67} & 16.63 & 8.63 & 6.59 & 6.76 & 0.50 & 8.04 & 8.24 \\
PFLD-MNv2-1.0      & 6.14 & \best{16.50} & 16.35 & 8.51 & 5.78 & 6.34 & 0.25 & 5.82 & 7.22 \\
PipNet-R18         & 17.71 & 24.66 & \best{24.86} & 7.08 & 2.81 & 4.94 & 0.07 & 2.28 & 5.53 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{BS=1 end-to-end latency across all available device/backend variants. E2E latency includes detection, face cropping, landmark inference, and surrounding pipeline work; lower is better. Bold marks the best value in each row.}
\label{tab:cross-device-bs1-e2e}
\scriptsize
\setlength{\tabcolsep}{2.4pt}
\renewcommand{\arraystretch}{1.12}
\resizebox{\linewidth}{!}{%
...
Model & \makecell{PC\\{}PyTorch} & \makecell{PC\\{}XNN FP32} & \makecell{PC\\{}XNN INT8} & \makecell{iPhone\\{}Core ML} & \makecell{iPhone\\{}XNN FP32} & \makecell{iPhone\\{}XNN INT8} & \makecell{iPhone\\{}Portable} & \makecell{Redmi\\{}XNN FP32} & \makecell{Redmi\\{}XNN INT8} \\
\midrule
MobileFaceNet      & 181.94 & 60.43 & \best{59.64} & 107.35 & 177.74 & 169.33 & 3374.61 & 277.18 & 272.20 \\
PFLD-MNv2-0.25     & -- & 59.92 & \best{59.92} & 107.89 & 135.38 & 135.90 & 1999.97 & 107.34 & 104.81 \\
PFLD-MNv2-1.0      & 162.76 & \best{60.37} & 61.01 & 109.89 & 160.36 & 141.19 & 3962.29 & 155.65 & 121.56 \\
PipNet-R18         & 56.38 & 40.50 & \best{40.19} & 133.35 & 338.35 & 186.01 & 13698.42 & 421.52 & 164.59 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{BS=1 landmark-inference latency across all available device/backend variants. This table isolates only the landmark model forward pass, so it explains whether the model or another pipeline stage dominates the E2E result; lower is better. Bold marks the best value in each row.}
...
Model & \makecell{PC\\{}PyTorch} & \makecell{PC\\{}XNN FP32} & \makecell{PC\\{}XNN INT8} & \makecell{iPhone\\{}Core ML} & \makecell{iPhone\\{}XNN FP32} & \makecell{iPhone\\{}XNN INT8} & \makecell{iPhone\\{}Portable} & \makecell{Redmi\\{}XNN FP32} & \makecell{Redmi\\{}XNN INT8} \\
\midrule
MobileFaceNet      & 45.40 & 13.14 & 12.76 & \best{0.60} & 11.30 & 9.00 & 464.74 & 184.33 & 178.39 \\
PFLD-MNv2-0.25     & -- & 1.53 & 1.30 & 0.34 & 0.49 & \best{0.26} & 44.71 & 9.99 & 7.10 \\
PFLD-MNv2-1.0      & 17.69 & 3.96 & 2.23 & \best{0.49} & 4.98 & 1.47 & 577.76 & 61.54 & 23.78 \\
PipNet-R18         & 12.55 & 47.46 & 11.35 & \best{1.43} & 44.45 & 9.66 & 3847.09 & 351.97 & 97.46 \\
\bottomrule
\end{tabular}
}
\end{table}

Tables~\ref{tab:cross-device-bs1-fps},
\ref{tab:cross-device-bs1-e2e}, and
\ref{tab:cross-device-bs1-landmark} together answer three different questions.
Table~\ref{tab:cross-device-bs1-fps} asks which complete benchmark run processes
more frames per second. PC XNNPACK is the strongest reference for all four models,
but the best mobile backend is not always XNNPACK: on iPhone, Core ML reaches
$7$--$9$ run FPS across the model set and is much faster than iPhone portable
execution. Redmi has only the XNNPACK rows in this benchmark, and its INT8 rows
usually beat its FP32 rows except where the surrounding pipeline hides most of
the landmark-model gain.

Table~\ref{tab:cross-device-bs1-e2e} explains why run FPS alone is not enough.
It answers the user-facing question: ``how long until this image has a landmark
result?'' PC XNNPACK stays near $40$--$61$~ms, iPhone Core ML stays around
$107$--$133$~ms, and iPhone portable is too slow for deployment. Redmi E2E
latency improves substantially with INT8 for the heavier models, especially
PipNet-R18 ($421.52 \rightarrow 164.59$~ms). Table~\ref{tab:cross-device-bs1-landmark}
then separates the landmark forward pass from the rest of the pipeline. Core ML
has the smallest landmark-only latency on iPhone, but the E2E time remains much
higher than the sub-millisecond landmark pass, which means detector and
application overhead dominate once the landmark model is highly optimized.

\section{Mobile-only BS=1 comparison}

\begin{figure}[htbp]
    \centering
    \caption{BS=1 XNNPACK measured run throughput on iPhone 14 and Redmi; higher run FPS is better.}
    \label{fig:mobile-only-bs1-xnnpack-fps}
    \includegraphics[width=0.95\linewidth]{images/benchmarks/combined/combined_01_bs1_xnnpack_speed.png}
\end{figure}

\begin{table}[htbp]
\centering
\caption{BS=1 mobile-only XNNPACK measured run throughput. This table accompanies Figure~\ref{fig:mobile-only-bs1-xnnpack-fps} so the exact iPhone-vs-Redmi differences remain visible; higher run FPS is better. Bold marks the better phone for each row.}
\label{tab:mobile-only-bs1-xnnpack-fps}
\small
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{@{}llrr@{}}
\toprule
Model & Quant. & \makecell{iPhone 14\\{}run FPS} & \makecell{Redmi\\{}run FPS} \\
\midrule
PFLD-MNv2-0.25 & FP32 & 6.59 & \best{8.04} \\
PFLD-MNv2-0.25 & INT8 & 6.76 & \best{8.24} \\
PFLD-MNv2-1.0  & FP32 & 5.78 & \best{5.82} \\
PFLD-MNv2-1.0  & INT8 & 6.34 & \best{7.22} \\
PipNet-R18     & FP32 & \best{2.81} & 2.28 \\
PipNet-R18     & INT8 & 4.94 & \best{5.53} \\
\bottomrule
\end{tabular}
\end{table}

The chart shows the pattern
quickly: Redmi is ahead on most XNNPACK rows, while iPhone 14 is better for
PipNet-R18 FP32. The table gives the exact magnitude of those differences. The
Redmi advantage is largest for PFLD-MNv2-1.0 INT8 ($7.22$ vs $6.34$ run FPS),
but the phones are nearly tied for PFLD-MNv2-1.0 FP32 ($5.82$ vs $5.78$ run
FPS). This section should not be read as a full iPhone-vs-Android ranking,
because the iPhone-specific Core ML results later in the chapter use a different
Apple-accelerated backend.

\section{Focus-device BS=1 backend and quantization}

\begin{figure}[htbp]
    \centering
    \caption{Focus device: BS=1 measured run throughput by backend and quantization.}
...
\begin{table}[htbp]
\centering
\caption{Focus-device BS=1 XNNPACK metrics. Run FPS is measured benchmark-run throughput, LM inf. means landmark inference, E2E means end-to-end, and ms means milliseconds. Bold values mark the best value in each numeric column: highest run FPS and lowest latency/error.}
...
\toprule
Model & Quant. & \makecell{Run\\{}FPS} & \makecell{LM inf.\\{}[ms]} & \makecell{E2E\\{}[ms]} & NME \\
\midrule
MobileFaceNet      & FP32 & 3.39 & 184.33 & 277.18 & \best{0.0428} \\
MobileFaceNet      & INT8 & 3.46 & 178.39 & 272.20 & \best{0.0428} \\
PFLD-MNv2-0.25     & FP32 & 8.04 & 9.99   & 107.34 & 0.0978 \\
PFLD-MNv2-0.25     & INT8 & \best{8.24} & \best{7.10} & \best{104.81} & 0.1039 \\
...
PipNet-R18 gains 2.42$\times$ in run FPS ($2.28 \rightarrow 5.53$) and
its landmark inference drops from $352$~ms to $97$~ms (a 3.6$\times$
reduction); PFLD-MNv2-1.0 gains 1.24$\times$ in run FPS but 2.6$\times$ in
...
speed/accuracy trade-off in this benchmark: a 24\% run-FPS gain costs a 27\% NME
increase, which may or may not be acceptable depending on the actual use case.
PipNet-R18 INT8, by contrast, is a near-Pareto win. It has the largest speed
gain in the matrix with only a barely measurable accuracy loss, making it the
...
\begin{figure}[htbp]
    \centering
    \caption[Focus device batch scaling for PFLD-MNv2-1.0]{Focus device batch scaling for PFLD-MNv2-1.0 using XNNPACK variants.}
...
\begin{figure}[htbp]
    \centering
    \caption[Focus device batch scaling for PipNet ResNet18]{Focus device batch scaling for PipNet ResNet18 using XNNPACK variants.}
...
\begin{table}[htbp]
\centering
\caption{Focus-device batch scaling in measured run FPS. BS means batch size. Bold marks the highest run FPS in each batch-size column.}
...
\begin{tabular}{@{}llrrrr@{}}
\toprule
Model & Quant. & \makecell{Run FPS\\{}BS=1} & \makecell{Run FPS\\{}BS=2} & \makecell{Run FPS\\{}BS=4} & \makecell{Run FPS\\{}BS=8} \\
\midrule
PFLD-MNv2-1.0 & FP32 & 5.82 & 5.53 & 5.44 & 5.97 \\
PFLD-MNv2-1.0 & INT8 & \best{7.22} & \best{7.50} & \best{7.49} & \best{7.66} \\
...
\begin{table}[htbp]
\centering
\caption{Focus-device batch scaling in landmark-inference latency. LM means landmark inference and BS means batch size. Bold marks the lowest latency in each batch-size column.}
...
shape of the batch-scaling curves for the focus phone, while
Tables~\ref{tab:focus-batch-fps} and~\ref{tab:focus-batch-lm-ms} provide the
exact run-FPS and landmark-latency values.

Measured run FPS for PFLD-MNv2-1.0 stays in a tight $5.4$--$6.0$
range (FP32) and $7.2$--$7.7$ range (INT8) from BS=1 to BS=8, while PipNet-R18
hovers at $\sim$2.3 run FPS (FP32) and $\sim$5.4--5.7 run FPS (INT8). In contrast,
...
\begin{table}[htbp]
\centering
\caption{Three-device INT8 XNNPACK batch scaling for PFLD-MNv2-1.0 in measured run FPS; higher is better. Bold marks the best value in each batch-size column.}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Device & \makecell{Run FPS\\{}BS=1} & \makecell{Run FPS\\{}BS=2} & \makecell{Run FPS\\{}BS=4} & \makecell{Run FPS\\{}BS=8} \\
\midrule
PC        & \best{16.35} & \best{35.62} & \best{35.87} & \best{35.82} \\
iPhone 14 & 6.34 & 6.24 & 6.37 & 6.51 \\
Redmi     & 7.22 & 7.50 & 7.49 & 7.66 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Three-device INT8 XNNPACK batch scaling for PipNet-R18 in measured run FPS; higher is better. Bold marks the best value in each batch-size column.}
...
Device & \makecell{Run FPS\\{}BS=1} & \makecell{Run FPS\\{}BS=2} & \makecell{Run FPS\\{}BS=4} & \makecell{Run FPS\\{}BS=8} \\
\midrule
PC        & \best{24.86} & \best{24.97} & \best{24.96} & \best{24.78} \\
iPhone 14 & 4.94 & 5.02 & 4.96 & 5.00 \\
Redmi     & 5.53 & 5.34 & 5.52 & 5.66 \\
\bottomrule
\end{tabular}
\end{table}
...
For PFLD-MNv2-1.0, the PC benefits strongly from moving beyond BS=1: run FPS
more than doubles from $16.35$ to about $35.8$. Both phones remain much flatter:
iPhone 14 stays around $6.2$--$6.5$ run FPS and Redmi around $7.2$--$7.7$ run
FPS. For PipNet-R18, even the PC is almost flat in run FPS, and the phones move
only slightly. This is an important practical lesson: optimizing the landmark
forward pass can be real and measurable, but measured run FPS may barely move if
another stage, such as detection or image handling, has become the bottleneck.

\section{Quantization deltas and cumulative error behavior}

\begin{figure}[htbp]
    \centering
    \caption{Focus device: INT8 vs FP32 measured run-throughput comparison at BS=1.}
...
\begin{figure}[htbp]
    \centering
    \caption{Focus device cumulative NME histogram for BS=1 XNNPACK FP32 and INT8 runs. Curves that rise earlier and stay higher indicate that more images have small normalized landmark error.}
...
\begin{figure}[htbp]
    \centering
    \caption{Focus device cumulative RMSE histogram for BS=1 XNNPACK FP32 and INT8 runs. This uses the same cumulative logic as the NME chart, but reports root-mean-square landmark error.}
    \label{fig:focus-cdf-rmse}
    \includegraphics[width=0.95\linewidth]{../benchmarks/results/combined/charts/focus_08_cumulative_hist_rmse_bs1.png}
\end{figure}

\begin{table}[htbp]
\centering
\caption{BS=1 PC INT8 and focus-device INT8 metrics. Bold marks the best value in each column: highest measured run FPS and lowest E2E latency.}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Model & \makecell{PC INT8\\{}run FPS} & \makecell{Focus INT8\\{}run FPS} & \makecell{PC E2E\\{}[ms]} & \makecell{Focus E2E\\{}[ms]} \\
\midrule
MobileFaceNet      & 16.75 & 3.46 & 59.64 & 272.20 \\
PFLD-MNv2-0.25     & 16.63 & \best{8.24} & 59.92 & \best{104.81} \\
PFLD-MNv2-1.0      & 16.35 & 7.22 & 61.01 & 121.56 \\
PipNet-R18         & \best{24.86} & 5.53 & \best{40.19} & 164.59 \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:focus-int8-speed} summarises the BS=1 INT8/FP32 run-FPS ratio per
model and makes the two-regime story visually explicit. A bar near
$1.0\times$ means that INT8 and FP32 are practically the same speed for the measured
benchmark run. A bar above $1.0\times$ means INT8 is faster. MobileFaceNet and
PFLD-MNv2-0.25 sit almost on the unity line ($1.02\times$ and $1.02\times$),
PFLD-MNv2-1.0 reaches $1.24\times$, and PipNet-R18 is the clear outlier at
$2.42\times$. Therefore, quantization is not a universal speed button; it helps
most when the model has enough quantizable computation for the backend to
benefit.

Figure~\ref{fig:focus-int8-accuracy-delta} must be read in the opposite
direction from the speedup plot. Here, values near zero are good because they
...
therefore: prefer INT8 for PipNet-R18, treat MobileFaceNet INT8 as a small but
mostly free win, and evaluate the PFLD-family accuracy/throughput trade-off
against the needs of the actual application.
...
\begin{figure}[htbp]
    \centering
    \caption{iPhone 14: BS=1 median measured run FPS (device-focused chart pack).}
...
\begin{figure}[htbp]
    \centering
    \caption{iPhone 14: BS=1 measured run throughput by backend and quantization.}
...
\begin{figure}[htbp]
    \centering
    \caption{iPhone 14: INT8 and FP32 measured run-throughput comparison at BS=1.}
...
\begin{table}[htbp]
\centering
\caption{iPhone 14 BS=1 measured run throughput by backend. XNN means XNNPACK. Bold marks the highest run FPS in each backend column.}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Model & \makecell{Core ML\\{}run FPS} & \makecell{XNN FP32\\{}run FPS} & \makecell{XNN INT8\\{}run FPS} & \makecell{Portable\\{}run FPS} \\
\midrule
MobileFaceNet      & 8.45 & 5.14 & 5.38 & 0.30 \\
PFLD-MNv2-0.25     & \best{8.63} & \best{6.59} & \best{6.76} & \best{0.50} \\
...
\begin{table}[htbp]
\centering
\caption{iPhone 14 BS=1 end-to-end latency by backend. Bold marks the lowest latency in each backend column.}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Model & \makecell{Core ML\\{}E2E [ms]} & \makecell{XNN FP32\\{}E2E [ms]} & \makecell{XNN INT8\\{}E2E [ms]} & \makecell{Portable\\{}E2E [ms]} \\
\midrule
MobileFaceNet      & \best{107.35} & 177.74 & 169.33 & 3374.61 \\
PFLD-MNv2-0.25     & 107.89 & \best{135.38} & \best{135.90} & \best{1999.97} \\
...
ML delivers a remarkably uniform $7.1$--$8.6$ run FPS across all four landmark
...
the Xiaomi: PipNet-R18 is again the largest winner (run FPS $2.81 \rightarrow
4.94$, $1.76\times$; landmark inference $44.5 \rightarrow 9.7$~ms,
$4.6\times$), PFLD-MNv2-1.0 moves modestly ($1.10\times$ run FPS, $3.4\times$
...
\begin{table}[htbp]
\centering
\caption{iPhone 14 batch scaling in measured run FPS. BS means batch size. Bold marks the highest run FPS in each batch-size column.}
...
PFLD-MNv2-1.0 & INT8 & \best{6.34} & \best{6.24} & \best{6.37} & \best{6.51} \\
...
amortised away. Measured run FPS, however, only drifts from $5.78$ to $5.91$
for PFLD-MNv2-1.0 FP32 and from $2.81$ to $2.93$ for PipNet-R18 FP32.

\begin{table}[htbp]
\centering
\caption{BS=1 PC INT8 and iPhone 14 INT8 metrics. Bold marks the best value in each column: highest measured run FPS and lowest E2E latency.}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Model & \makecell{PC INT8\\{}run FPS} & \makecell{iPhone INT8\\{}run FPS} & \makecell{PC E2E\\{}[ms]} & \makecell{iPhone E2E\\{}[ms]} \\
\midrule
MobileFaceNet      & 16.75 & 5.38 & 59.64 & 169.33 \\
PFLD-MNv2-0.25     & 16.63 & \best{6.76} & 59.92 & \best{135.90} \\
PFLD-MNv2-1.0      & 16.35 & 6.34 & 61.01 & 141.19 \\
PipNet-R18         & \best{24.86} & 4.94 & \best{40.19} & 186.01 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item implementation and comparison of mobile inference paths on both iOS
          and Android,
    \item evaluation of real-time-oriented metrics (ms/frame and run FPS),
...
\begin{itemize}
    ...
    \item INT8 can provide major latency and run-FPS gains for heavier landmark
