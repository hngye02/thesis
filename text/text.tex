...
\begin{figure}[htbp]
    ...
    \caption{Face-processing pipeline on a real Annotated Faces in the Wild (AFW) sample image \cite{zhu2012facedetectionwild}: the first panel shows the input crop, the second panel draws the face region computed as the tight bounding box around the AFW landmark points, and the third panel overlays the annotated landmark points.}
...
In this thesis, AFW annotations are used as the ground-truth reference for
landmark evaluation. The annotations originate from the face detection and pose
estimation work of Zhu and Ramanan \cite{zhu2012facedetectionwild}.
...
ground-truth annotations from the AFW dataset \cite{zhu2012facedetectionwild}. For $N$ evaluated images and $K$ landmarks,
