...
\begin{figure}[htbp]
    ...
    \caption{Face-processing pipeline on a real Annotated Faces in the Wild (AFW) sample image \cite{zhu2012facedetectionwild} with 300-W/iBUG landmark annotations \cite{sagonas2013_300w}: the first panel shows the input crop, the second panel draws the face region computed as the tight bounding box around the AFW landmark points, and the third panel overlays the annotated landmark points.}
...
for this explanatory figure, the rectangle is not a detector output, but the
tight bounding box obtained from the minimum and maximum coordinates of the AFW
ground-truth landmark points for the sample image
\cite{sagonas2013_300w}. A deployed system would obtain this region from
a detector before cropping and normalizing the face. The third panel shows the
annotated landmark points. These
...
implemented with ExecuTorch, a PyTorch edge-deployment runtime for exporting and
running neural-network models on mobile and embedded devices
\cite{executorch_how,executorch_arch}. The benchmark must support equivalent
iOS and Android applications, consistent measurement conditions, and validation
of quality metrics.
...
\begin{enumerate}
    ...
    \item \textbf{Model selection + test inputs:} Chapter~2 (model section)
          ...
          the reproducible AFW (Annotated Faces in the Wild)-based input protocol \cite{zhu2012facedetectionwild,sagonas2013_300w} used for quantitative
          comparison.
...
In this thesis, a backend means the runtime implementation that executes the
model operators on a particular hardware path, for example by using optimized CPU
kernels or by delegating supported operations to an accelerator. The measured
backends are Core ML (Apple machine learning framework), XNNPACK, and portable
...
known values before quantization are the FP32 weights and representative FP32
activation values observed during calibration. The values that must be found are
the quantized integers and the mapping parameters for each quantized tensor:
scale $s$ and zero-point $z$. The main target is INT8 execution, where a real
value $x$ is represented by an integer $q$:
\begin{equation}
...
For a tensor $T$, calibration observes a floating-point range
\([x_{\min}, x_{\max}]\). Quantization then chooses $s$ and $z$ so that this
observed real range is mapped to the available integer range
\([q_{\min}, q_{\max}]\). In the common asymmetric case, the scale can be written
as
\begin{equation}
...
\end{equation}
with $z$ clipped to the valid integer range if necessary
\cite{jacob2018quantization,krishnamoorthi2018quantizing}.
...
\cite{torchao_pt2e}. For each observed tensor, these recorded
\(x_{\min}\) and \(x_{\max}\) values are then converted into the corresponding
scale $s$ and zero-point $z$. If calibration data is too narrow, later real inputs may fall
...
Post-training quantization (PTQ) typically has three steps:
\begin{enumerate}
...
The selected models are convolutional neural networks (CNNs), not
transformer-based vision models such as Vision Transformers (ViTs). This choice
matches the mobile deployment goal, because the selected architectures mostly use
convolutions, depthwise/mobile backbones, and small prediction heads that are
well supported by mobile runtimes. RetinaFace is a strong one-stage CNN detector;
the MNet0.25 variant uses a MobileNet-style lightweight backbone to reduce
compute while preserving robust detection quality
\cite{deng2020retinaface,retinaface_repo}. FaceBoxes is also a CNN detector, but
it is explicitly designed for CPU real-time operation
\cite{zhang2017faceboxes,faceboxesv2_repo}.

Other detector families also exist, including YOLO-style one-stage object
detectors, SSD-like detectors, and multi-stage face detectors. They are not the
main focus here because the benchmark uses a fixed set of detector/landmark
pairs for comparable mobile measurements.

For landmarks, PFLD (Practical Facial Landmark Detector) is a compact CNN
landmark regressor that emphasizes speed \cite{guo2019pfld,pfld68_repo}.
MobileFaceNet provides a lightweight backbone originally developed for mobile
face recognition and often reused as a CNN feature extractor for landmark
regression \cite{chen2018mobilefacenets,mobilefacenet_pytorch_repo}. PIPNet
(Pixel-in-Pixel Network) combines a convolutional backbone, here represented by
the ResNet18 variant, with heatmap-style reasoning and low-resolution efficient
heads for practical landmark localization
...
In this thesis, AFW images are used with 68-point landmark annotations as the
ground-truth reference for landmark evaluation. The images come from the AFW
dataset introduced by Zhu and Ramanan, while the landmark markup follows the
300-W/iBUG facial landmark challenge annotations
\cite{zhu2012facedetectionwild,sagonas2013_300w}.
...
same workflow on Android and iOS. TFLite is a strong mobile and embedded
runtime for models in TensorFlow Lite format \cite{tensorflow_lite}, and
...
model is loaded by the Android and iOS benchmark applications
...
\begin{enumerate}
    \item export a PyTorch model to a computational graph form,
...
Each node represents an action, such as reading an input
...
\begin{enumerate}
    ...
    \item creation of a serialized \texttt{.pte} artifact, where the artifact
...
program and tensor data into \texttt{.pte} and \texttt{.ptd} files respectively
...
A \texttt{.pte} file stores an already exported and lowered execution program. 
That program can include delegate segments, portable fallback segments, constants, and the method entry points
...
\begin{itemize}
    ...
    \item \textbf{Core ML delegate}: Apple backend path that can dispatch to CPU,
          GPU, or NPU depending on model and platform capabilities
...
XNNPACK is a low-level library of optimized neural-network operator kernels \cite{xnnpack_repo}. 
It targets CPU instruction sets on ARM and x86 systems, including Android, iOS, macOS, Linux, Windows, and
...
the tensor dimensions are ordered as batch-size, height, width, and channels
...
\begin{enumerate}
    \item measure latency, throughput, and accuracy consistently on iOS and
          Android,
    \item preserve raw outputs needed for analysis of the results.
\end{enumerate}

\section{Evaluated pipeline}

Each benchmark executes an end-to-end pipeline:
\begin{enumerate}
...
Annotated Faces in the Wild (AFW)-based images with 300-W/iBUG landmark
annotations are used for accuracy evaluation
\cite{zhu2012facedetectionwild,sagonas2013_300w}.
...
timestamps, respectively, and \(\ell_i^{\mathrm{lm}}\) is the resulting latency in
milliseconds.
...
median per-image landmark-inference latency as a function of resolved landmark
batch size ($b \in \{1,2,4,8\}$ in the curated thesis set). For a batch run, the
measured total landmark-forward latency is divided by the batch size \(b\), so
the plotted value is the latency for one image rather than the latency of the
whole batch. This directly shows how the landmark model forward pass responds to
batching.

\subsection{Landmark accuracy}

Accuracy is recomputed offline from exported per-image predicted landmarks and
ground-truth annotations for AFW images with 300-W/iBUG landmark markup
\cite{zhu2012facedetectionwild,sagonas2013_300w}. For $N$ evaluated images and $K$ landmarks,
...
Root-mean-square error (RMSE) is also reported for landmark accuracy. For the
same \(N\) evaluated images and \(K\) landmarks, the per-image landmark RMSE is
computed in image-coordinate units as
\begin{equation}
\mathrm{RMSE}_{n} =
\sqrt{\frac{1}{K}\sum_{k=1}^{K}
\lVert \hat{\mathbf{p}}_{n,k} - \mathbf{p}_{n,k} \rVert_2^2},
\end{equation}
and the dataset-level RMSE is
\begin{equation}
\mathrm{RMSE} = \frac{1}{N}\sum_{n=1}^{N}\mathrm{RMSE}_{n}.
\end{equation}
Here, \(n\) indexes the image, \(k\) indexes the landmark, \(N\) is the number of
evaluated images, \(K\) is the number of landmarks per image,
\(\hat{\mathbf{p}}_{n,k}\) is the predicted two-dimensional landmark coordinate,
\(\mathbf{p}_{n,k}\) is the matching ground-truth coordinate, and
\(\lVert\cdot\rVert_2\) is the Euclidean distance between the two points. Unlike
NME, this RMSE is not divided by the inter-ocular distance, so it remains in the
same coordinate units as the landmarks.
...
\end{equation}
where \(e_n\) is the selected per-image error value for image \(n\), \(\tau\) is
the error threshold on the horizontal axis, \(N\) is the number of evaluated
images, and \(\mathbf{1}[\cdot]\) is one when the condition is true and zero
otherwise.
...
from image predictions. This allows for more flexible and transparent analysis, 
...
\begin{figure}[htbp]
    ...
    \caption{Landmark-inference latency at BS=1: PC vs Redmi median forward-pass time in milliseconds. Lower bars are better.}
...
\begin{table}[htbp]
\centering
\caption{Landmark-inference latency at BS=1 across all available device/backend variants. Values are milliseconds and isolate only the landmark model forward pass; lower is better. Bold marks the lowest latency for each model.}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
Device/backend & MobileFaceNet & \makecell{PFLD-\\{}MNv2-0.25} & \makecell{PFLD-\\{}MNv2-1.0} & PipNet-R18 \\
\midrule
PC PyTorch          & 45.40 & --   & 17.69 & 12.55 \\
PC XNNPACK FP32     & 13.14 & 1.53 & 3.96  & 47.46 \\
PC XNNPACK INT8     & 12.76 & 1.30 & 2.23  & 11.35 \\
iPhone Core ML      & \best{0.60} & 0.34 & \best{0.49} & \best{1.43} \\
iPhone XNNPACK FP32 & 11.30 & 0.49 & 4.98  & 44.45 \\
iPhone XNNPACK INT8 & 9.00  & \best{0.26} & 1.47 & 9.66 \\
iPhone Portable     & 464.74 & 44.71 & 577.76 & 3847.09 \\
Redmi XNNPACK FP32  & 184.33 & 9.99 & 61.54 & 351.97 \\
Redmi XNNPACK INT8  & 178.39 & 7.10 & 23.78 & 97.46 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
    ...
    \caption{Landmark-inference latency at BS=1 with XNNPACK on iPhone 14 and Redmi. The chart uses only mobile devices and only the landmark forward pass; lower latency is better.}
    \label{fig:mobile-only-bs1-xnnpack-latency}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Landmark-inference latency at BS=1 for mobile-only XNNPACK runs; lower latency is better. Bold marks the lowest latency in each row.}
...
\section{Redmi backend and quantization at BS=1}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{../benchmarks/results/combined/charts/focus_01_bs1_landmark_latency_backend_quant.png}
    \caption{Landmark-inference latency on Redmi at BS=1 by backend and quantization. This chart focuses on the landmark model forward pass; lower latency is better.}
...
\begin{figure}[htbp]
    ...
    \caption{NME accuracy on Redmi at BS=1 by backend and quantization. Lower is better.}
    \label{fig:focus-bs1-nme}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Landmark latency and accuracy on Redmi at BS=1 with XNNPACK. LM inf. means landmark inference and ms means milliseconds. Bold text values mark the lower latency and lower error within each model pair.}
...
\begin{figure}[htbp]
    ...
    \caption[Per-image landmark-latency batch scaling on Redmi for PFLD-MNv2-1.0]{Per-image landmark-inference latency batch scaling on Redmi for PFLD-MNv2-1.0 using XNNPACK variants. Total batch latency is divided by batch size; lower latency is better.}
...
\begin{figure}[htbp]
    ...
    \caption[Per-image landmark-latency batch scaling on Redmi for PipNet ResNet18]{Per-image landmark-inference latency batch scaling on Redmi for PipNet ResNet18 using XNNPACK variants. Total batch latency is divided by batch size; lower latency is better.}
    \label{fig:focus-batch-pipnet}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Per-image landmark-inference latency batch scaling on Redmi. LM means landmark inference and BS means batch size. Values are total batch latency divided by BS. Bold marks the lowest latency in each batch-size column.}
...
shape of the per-image landmark-latency batch-scaling curves for Redmi, while
Table~\ref{tab:focus-batch-lm-ms} provides the exact values. Each plotted value
is computed by dividing the total landmark-forward latency of the batch by the
batch size. The landmark forward pass improves strongly as the resolved landmark
batch grows:
...
\begin{table}[htbp]
\centering
\caption{Per-image landmark-inference latency batch scaling for PFLD-MNv2-1.0 with INT8 XNNPACK across three devices. Values are total batch latency divided by BS; lower is better. Bold marks the best value in each batch-size column.}
...
\begin{table}[htbp]
\centering
\caption{Per-image landmark-inference latency batch scaling for PipNet-R18 with INT8 XNNPACK across three devices. Values are total batch latency divided by BS; lower is better. Bold marks the best value in each batch-size column.}
...
\begin{figure}[htbp]
    ...
    \caption{NME difference between INT8 and FP32 on Redmi at BS=1.}
...
\begin{figure}[htbp]
    ...
    \caption{Cumulative NME histogram on Redmi for BS=1 XNNPACK FP32 and INT8 runs. Curves that rise earlier and stay higher indicate that more images have small normalized landmark error.}
...
\begin{figure}[htbp]
    ...
    \caption{Cumulative RMSE histogram on Redmi for BS=1 XNNPACK FP32 and INT8 runs. This uses the same cumulative logic as the NME chart, but reports root-mean-square landmark error.}
    \label{fig:focus-cdf-rmse}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Landmark-inference latency at BS=1 for PC INT8 and Redmi INT8. Bold marks the lower latency in each row.}
...
\begin{figure}[htbp]
    ...
    \caption{Landmark-inference latency on iPhone 14 at BS=1, reported as median forward-pass time.}
    \label{fig:iphone-simple-latency}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Landmark-inference latency on iPhone 14 at BS=1 by backend. XNN means XNNPACK. Bold marks the lowest latency in each row.}
...
\begin{figure}[htbp]
    ...
    \caption{Per-image landmark-inference latency batch scaling on iPhone 14 for PFLD-MNv2-1.0. Total batch latency is divided by batch size.}
...
\begin{figure}[htbp]
    ...
    \caption{Per-image landmark-inference latency batch scaling on iPhone 14 for PipNet ResNet18. Total batch latency is divided by batch size.}
    \label{fig:iphone-batch-pipnet}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Per-image landmark-inference latency batch scaling on iPhone 14. LM means landmark inference and BS means batch size. Values are total batch latency divided by BS. Bold marks the lowest latency in each batch-size column.}
...
\begin{table}[htbp]
\centering
\caption{Landmark-inference latency at BS=1 for PC INT8 and iPhone 14 INT8. Bold marks the lower latency in each row.}
