...
from a single average. For each evaluated image, the offline script computes an
error value such as NME or RMSE. The script then asks a simple question for many
thresholds on the horizontal axis: ``what fraction of images has error less than
or equal to this threshold?'' The vertical axis is that fraction.
...
means consistent behavior and a long right tail means that some images are
outliers where the landmark prediction failed or became noticeably worse.
